...
class ImportForm(forms.Form):
    uploads = MultiFixtureField(label=_("Cargar"), required=False)

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if settings.SMUGGLER_FIXTURE_DIR:
            self.fields["store"] = forms.BooleanField(
                label=_("Guardar en el directorio"),
                required=True,
                help_text=(
                    _('Las salvas serán almacenadas en "%(fixture_dir)s".')
    ...
    def clean(self):
        ...
        if settings.SMUGGLER_FIXTURE_DIR:
            ...
            if not uploads and not picked_files:
                raise forms.ValidationError(
                    _("Al menos un archivo debe ser" " cargado o seleccionado.")
                )
